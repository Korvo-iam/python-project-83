...
def test_add_url(monkeypatch):
    ...
    client = app.test_client()
    monkeypatch.setattr('page_analyzer.sql_commands.check_if_in_db', lambda url: None)
    monkeypatch.setattr('page_analyzer.sql_commands.add_in_db', lambda url: 999)
    monkeypatch.setattr('page_analyzer.sql_commands.return_url_checks', lambda id:(mock_url,[]))
    response = client.post('/', data={'url': 'https://example.com'}, follow_redirects=True)
    assert response.status_code == 200
...
def test_url_detail(monkeypatch):
    ...
    monkeypatch.setattr('page_analyzer.sql_commands.return_url_checks', lambda id: (mock_url, mock_checks))
